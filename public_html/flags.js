...
// This table will be sorted during initialization. 
...
var ICAO_Ranges = [
        ...
        { start: 0xF00000, end: 0xF07FFF, country: "ICAO (temporary assignments)", flag_image: null },
        { start: 0x899000, end: 0x8993FF, country: "ICAO (special use)", flag_image: null },
        { start: 0xF09000, end: 0xF093FF, country: "ICAO (special use)", flag_image: null }
];

var ICAO_U_Ranges = [
        // Block assignments mentioned in Chapter 9 section 4, at the end so they are only used if
        // nothing above applies
        { start: 0x200000, end: 0x27FFFF, country: "Unassigned (AFI region)", flag_image: null },
        { start: 0x280000, end: 0x28FFFF, country: "Unassigned (SAM region)", flag_image: null },
        { start: 0x500000, end: 0x5FFFFF, country: "Unassigned (EUR / NAT regions)", flag_image: null },
        { start: 0x600000, end: 0x67FFFF, country: "Unassigned (MID region)", flag_image: null },
        { start: 0x680000, end: 0x6FFFFF, country: "Unassigned (ASIA region)", flag_image: null },
        { start: 0x900000, end: 0x9FFFFF, country: "Unassigned (NAM / PAC regions)", flag_image: null },
        { start: 0xB00000, end: 0xBFFFFF, country: "Unassigned (reserved for future use)", flag_image: null },
        { start: 0xD00000, end: 0xDFFFFF, country: "Unassigned (reserved for future use)", flag_image: null },
        { start: 0xEC0000, end: 0xEFFFFF, country: "Unassigned (CAR region)", flag_image: null },
        { start: 0xF00000, end: 0xFFFFFF, country: "Unassigned (reserved for future use)", flag_image: null }
];
...
// Find the range using a binary search. Array must already be sorted.
function rangeLookup(array, item) {
        var min = 0;
        ...
        while (min <= max) {
                ...
                if (item >= array[guess].start && item <= array[guess].end)
                        return array[guess];
                else
                        if (item < array[guess].start)
                                max = guess - 1;
                        else
                                min = guess + 1;
        }
...
function findICAORange(icao) {
        ...
        var range = rangeLookup(ICAO_Ranges, hexa);
        if (range) return range;

        range = rangeLookup(ICAO_U_Ranges, hexa);
        if (range) return range;
